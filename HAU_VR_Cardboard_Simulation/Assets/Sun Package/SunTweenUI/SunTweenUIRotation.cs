...
{
    public class SunTweenUIRotation : SunTweenUI
    {
        #region Fields
        ...
        private void SetTweenForward(Vector3 f, Vector3 t)
        {
            ...
            switch (styleForward)
            {
                case Style.Once:
                    ...
                    break;
                case Style.Loop:
                    MainTween = mainTarget.DOLocalRotate(t, duration)
                        .SetDelay(delayWhen == DelayWhen.Both || delayWhen == DelayWhen.Forward ? delay : 0)
        ...
        public override void SetCurrentValueToStart()
        {
            ...
            fromValue = target.localRotation.eulerAngles;
        }
        ...
        {
            var target = GetComponent<RectTransform>();
            toValue = target.localRotation.eulerAngles;
        }
        ...
        {
            var target = GetComponent<RectTransform>();
            target.localRotation = Quaternion.Euler(fromValue);
        }
        ...
        {
            var target = GetComponent<RectTransform>();
            target.localRotation = Quaternion.Euler(toValue);
        }
