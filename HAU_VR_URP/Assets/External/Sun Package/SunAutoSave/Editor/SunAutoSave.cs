...
{
    [CustomEditor(typeof(SunAutoSaveConfig))]
    public class SunAutoSave : Editor
    {
        private static SunAutoSaveConfig _config;
        ...
        private static void FetchConfig()
        {
            while (true)
            {
                ...
                if (path == null)
                {
                    AssetDatabase.CreateAsset(CreateInstance<SunAutoSaveConfig>(), $"Assets/{nameof(SunAutoSaveConfig)}.asset");
                    Debug.Log("A config file has been created at the root of your project.<b> You can move this anywhere you'd like.</b>");
                    continue;
                }

                _config = AssetDatabase.LoadAssetAtPath<SunAutoSaveConfig>(path);
        ...
        {
            var paths = AssetDatabase.FindAssets(nameof(SunAutoSaveConfig)).Select(AssetDatabase.GUIDToAssetPath)
        ...
        public static void ShowConfig()
        {
            ...
            var path = GetConfigPath();
            EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath<SunAutoSaveConfig>(path).GetInstanceID());
        }
